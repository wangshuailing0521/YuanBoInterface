...
{
    [Description("应收单返回快递信息执行计划")]
    public class ReceivableKDSchedule: IScheduleService
    {
        Context _ctx;
        ...
        void ToInterface()
        {
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                if (string.IsNullOrWhiteSpace(interfaceUrl.FReceivableKDUrl))
                {
                ...
                sb.AppendLine($@"接口名称：应收单返回快递信息API");
                try
                {
        ...
        void Require(DynamicObject item, StringBuilder sb)
        {
            ...
            url = $@"{interfaceUrl.FReceivableKDUrl}/{billNo}";
        ...
        DynamicObjectCollection GetData()
        {
            string sql = $@"
                SELECT  A.FBILLNO
                 ...
                 WHERE  1=1
                   AND  ISNULL(A.F_PAEZ_Text2,'') <> ''
                   AND  ISNULL(A.FKDStatus,'0') = '0'";
            DynamicObjectCollection data
