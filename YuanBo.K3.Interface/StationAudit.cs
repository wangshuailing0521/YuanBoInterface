...
{
    [Description("岗位审核插件")]
    public class StationAudit: AbstractOperationServicePlugIn
    {
        string url = "http://api.kingdee.114study.com/api/king_dee_stations";
        string crmId = "";
        ...
        public void Add(StringBuilder sb, DynamicObject billObj)
        {
            ...
            DynamicObject dept = billObj["FDept"] as DynamicObject;

            var requestInfo = new
            {
                king_dee_station_no = billObj["Number"].ToString(),
                king_dee_station_name = billObj["Name"].ToString(),
                king_dee_department_no = dept["Number"].ToString(),
            ...
            if (model["code"] != null)
            {
                ...
                else
                {
                    ...
                    SqlHelper.UpdateStation(this.Context, billObj["Id"].ToString(), crmId);
        ...
        public void Edit(StringBuilder sb, DynamicObject billObj)
        {
            ...
            url = $@"http://api.kingdee.114study.com/api/king_dee_stations/{number}";
            var requestInfo = new
            {
                king_dee_station_name = billObj["Name"].ToString(),
                king_dee_department_no = dept["Number"].ToString(),
